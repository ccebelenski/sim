...
import sim.device.SupportsOptions
  ...
  override def process(tokenArray: Array[String]): Boolean = {
    ...
    if (tokenArray.size != 1) Utils.outln(s"SIM: Please specify a machine.")
      ...
      AbstractMachine.services.find(am => am.getName == mn) match {
        ...
        case Some(x) =>
          if (sim.Console.simEnvironment.simMachine.isEmpty) {
            sim.Console.simEnvironment.simMachine = Some(x)
            x.init()
            Utils.outln(s"SIM: Machine $mn defined.")
          } else {
            Utils.outln(s"SIM: Machine is already defined: ${sim.Console.simEnvironment.simMachine.get.getName}")
          }
      }
      if (sim.Console.simEnvironment.simMachine.isEmpty) Utils.outln(s"SIM: Machine was not found. LIST MACHINES to get a list.")
    }
