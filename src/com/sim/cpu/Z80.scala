...
class Z80(isBanked: Boolean, override val machine: AbstractMachine) extends BasicCPU(isBanked, machine) {
  ...
  private final def SUBIDX(r1: Register8): Unit = {
    ...
    val acu: UByte = A.get8
    val sum: UInt = acu - temp
    AF((addTable(sum & 0x1ff) | cbitsZ80Table((acu ^ temp ^ sum) & 0x1ff)).intValue)
  }
  ...
  private final def SBCAIDX(r1: Register8, r2: Register8): Unit = {
    ...
    AF(addTable(sum & 0x1ff) | cbits2Z80Table((acu ^ temp ^ sum) & 0x1ff).intValue)
  }
...
*/
    ...
    (op & 0xc0) match {
      case (0x00) => // shift/rotate
        (op & 0x38) match {
          ...
          case (0x28) => // SRA
            temp = UByte((acu >> 1).byteValue) | UByte((acu & 0x80).byteValue())
            cbits = UInt(acu & 1)
            AF((AF & ~0xff) | rotateShiftTable(temp & 0xff) | {
              if (cbits == 0) 0 else 1
            })
          case (0x30) => // SLIA
            temp = UByte((acu << 1).byteValue) | UByte(1)
            cbits = UInt(acu & 0x80)
            AF((AF & ~0xff) | rotateShiftTable(temp & 0xff) | {
              if (cbits == 0) 0 else 1
            })
  ...
  private def ddprefix(op: Int): Unit = {
    op match {
      ...
      case (0xa4) => // AND IXH
        addTStates(9)
        AF(andTable((AF & IX.get16) >> 8 & 0xff))

      case (0xa5) => // AND IXL
        addTStates(9)
        AF(andTable(((AF >> 8) & IX.get16) & 0xff))
      ...
      case (0xac) => // XOR IXH
        addTStates(9)
        AF(xororTable(((AF ^ IX.get16) >> 8) & 0xff))

      case (0xad) => // XOR IXL
        addTStates(9)
        AF(xororTable(((AF >> 8) ^ IX.get16) & 0xff))
  ...
  private def fdprefix(op: Int): Unit = {
    ...
    op match {
      ...
      case (0x86) => // ADD A,(IY+dd)
        ...
        val temp: UByte = MMU.get8(adr.intValue)
        val acu: UByte = A.get8
        val sum: UInt = acu + temp
        AF((addTable(sum & 0x1ff) | cbitsZ80Table((acu ^ temp ^ sum) & 0x1ff)).intValue)
